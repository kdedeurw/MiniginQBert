...
void* operator new (size_t nbBytes)
{
	...
	MemoryAllocator::Tag* const tag = reinterpret_cast<MemoryAllocator::Tag*>(malloc(nbBytes + sizeof(MemoryAllocator::Tag)));
	tag->pool = nullptr;
	return tag + 1;
}
...
{
	if (nbBytes == 0) nbBytes = 1;
	MemoryAllocator::Tag* const tag = reinterpret_cast<MemoryAllocator::Tag*>(allocator.Acquire(nbBytes + sizeof(MemoryAllocator::Tag)));
	tag->pool = &allocator;
	return tag + 1;
}
...
GlobalMemoryPools::GlobalMemoryPools()
	...
	, m_SubjectAllocator{}
{
	m_ComponentAllocator.Initialize(1024 * 100);
	m_StackAllocator.Initialize(4096);
